...
class Product(Base):
    """Product catalog for content displays"""
    __tablename__ = "product"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    model_number: Mapped[str] = mapped_column(String(100), nullable=False, unique=True, index=True)
    ...
    rotation_duration: Mapped[int] = mapped_column(Integer, nullable=False, default=15)  # seconds
    display_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    created_by_user_id: Mapped[int] = mapped_column(Integer, ForeignKey("user.id", ondelete="SET NULL"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class ProductCategory(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    product_id: Mapped[int] = mapped_column(Integer, ForeignKey("product.id", ondelete="CASCADE"), nullable=False, index=True)
    category_id: Mapped[int] = mapped_column(Integer, ForeignKey("category.id", ondelete="CASCADE"), nullable=False, index=True)
    assigned_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
...
class ProductDeviceAssignment(Base):
    """Assigns products to specific devices"""
    __tablename__ = "product_device_assignment"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    device_id: Mapped[str] = mapped_column(String(36), ForeignKey("device.device_id", ondelete="CASCADE"), nullable=False, index=True)
    product_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("product.id", ondelete="CASCADE"), nullable=True, index=True)
    category_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("category.id", ondelete="CASCADE"), nullable=True, index=True)
    tag_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("tag.id", ondelete="CASCADE"), nullable=True, index=True)
    display_order: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    assigned_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)


class ProductLocationAssignment(Base):
    """Assigns products to locations (all devices at location inherit these)"""
    __tablename__ = "product_location_assignment"
...
class ProductView(Base):
    """Analytics: tracks when users view products"""
...
class ProductInteraction(Base):
    """Analytics: tracks user interactions (navigation, QR scans)"""
...
async def get_products(
    ...
    try:
        query = select(Product).order_by(Product.display_order, Product.model_number)
        ...
        # Fetch images, categories, and tags for each product
        products_data = []
        for product in products:
            ...
            products_data.append({
                ...
                "rotation_duration": product.rotation_duration,
...
async def create_product(
    ...
    try:
        ...
        existing = await session.execute(
            select(Product).where(Product.model_number == data.get('model_number'))
        )
        if existing.scalar_one_or_none():
            raise HTTPException(status_code=400, detail="Product with this model number already exists")
        ...
        product = Product(
            ...
            rotation_duration=data.get('rotation_duration', 15),
        ...
        for tag_id in data.get('tag_ids', []):
            session.add(ProductTag(product_id=product.id, tag_id=tag_id))

        await session.commit()

        print(f"[CREATE-PRODUCT] Created product: {product.model_number}")
...
async def update_product(
    ...
    try:
        ...
        product.rotation_duration = data.get('rotation_duration', product.rotation_duration)
        product.display_order = data.get('display_order', product.display_order)
        product.is_active = data.get('is_active', product.is_active)

        # Update categories if provided
        if 'category_ids' in data:
            await session.execute(delete(ProductCategory).where(ProductCategory.product_id == product_id))
            for category_id in data['category_ids']:
                session.add(ProductCategory(product_id=product_id, category_id=category_id))

        # Update tags if provided
        if 'tag_ids' in data:
            await session.execute(delete(ProductTag).where(ProductTag.product_id == product_id))
            for tag_id in data['tag_ids']:
                session.add(ProductTag(product_id=product_id, tag_id=tag_id))

        await session.commit()

        print(f"[UPDATE-PRODUCT] Updated product: {product.model_number}")
        return {"success": True}
...
async def delete_product(
    ...
    try:
        ...
        print(f"[DELETE-PRODUCT] Deleted product: {product.model_number}")
        return {"success": True}
...
async def upload_product_image(
    ...
    try:
        ...
        print(f"[UPLOAD-PRODUCT-IMAGE] Uploaded image for product {product_id}")
        return {"success": True, "image_id": product_image.id, "file_path": product_image.file_path}
...
async def delete_product_image(
    ...
    try:
        ...
        if image.file_path:
            file_path = os.path.join(os.path.dirname(UPLOAD_FOLDER), image.file_path.lstrip('/'))
        ...
        print(f"[DELETE-PRODUCT-IMAGE] Deleted image {image_id} for product {product_id}")
        return {"success": True}
...
@app.get("/api/devices/{device_id}/products")
async def get_device_products(
    device_id: str,
    device: Device = Depends(get_current_device),
    session: AsyncSession = Depends(get_async_session)
):
    """Get products assigned to a device (for Tizen content display)"""
    try:
        products_set = set()

        # Get directly assigned products
        direct_result = await session.execute(
            select(Product)
            .join(ProductDeviceAssignment, Product.id == ProductDeviceAssignment.product_id)
            .where(
                ProductDeviceAssignment.device_id == device_id,
                ProductDeviceAssignment.product_id.isnot(None),
                Product.is_active == True
            )
            .order_by(ProductDeviceAssignment.display_order, Product.display_order)
        )
        for product in direct_result.scalars().all():
            products_set.add(product.id)

        # Get products via device category assignments
        category_result = await session.execute(
            select(Product)
            .join(ProductCategory, Product.id == ProductCategory.product_id)
            .join(ProductDeviceAssignment, ProductCategory.category_id == ProductDeviceAssignment.category_id)
            .where(
                ProductDeviceAssignment.device_id == device_id,
                ProductDeviceAssignment.category_id.isnot(None),
                Product.is_active == True
            )
        )
        ...
        # Get products via device tag assignments
        tag_result = await session.execute(
            select(Product)
            .join(ProductTag, Product.id == ProductTag.product_id)
            .join(ProductDeviceAssignment, ProductTag.tag_id == ProductDeviceAssignment.tag_id)
            .where(
                ProductDeviceAssignment.device_id == device_id,
                ProductDeviceAssignment.tag_id.isnot(None),
                Product.is_active == True
            )
        )
        ...
        # Get location-level assignments (if device has no direct assignments)
        if device.location_id and len(products_set) == 0:
            # Direct location products
            location_direct_result = await session.execute(
                select(Product)
                .join(ProductLocationAssignment, Product.id == ProductLocationAssignment.product_id)
                .where(
                    ProductLocationAssignment.location_id == device.location_id,
                    ProductLocationAssignment.product_id.isnot(None),
                    Product.is_active == True
                )
            )
            for product in location_direct_result.scalars().all():
                products_set.add(product.id)

            # Location category products
            location_category_result = await session.execute(
                select(Product)
                .join(ProductCategory, Product.id == ProductCategory.product_id)
                .join(ProductLocationAssignment, ProductCategory.category_id == ProductLocationAssignment.category_id)
                .where(
                    ProductLocationAssignment.location_id == device.location_id,
                    ProductLocationAssignment.category_id.isnot(None),
                    Product.is_active == True
                )
            )
            for product in location_category_result.scalars().all():
                products_set.add(product.id)

            # Location tag products
            location_tag_result = await session.execute(
                select(Product)
                .join(ProductTag, Product.id == ProductTag.product_id)
                .join(ProductLocationAssignment, ProductTag.tag_id == ProductLocationAssignment.tag_id)
                .where(
                    ProductLocationAssignment.location_id == device.location_id,
                    ProductLocationAssignment.tag_id.isnot(None),
                    Product.is_active == True
                )
            )
            for product in location_tag_result.scalars().all():
                products_set.add(product.id)
        ...
        print(f"[GET-DEVICE-PRODUCTS] Error: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/products/analytics/view")
async def track_product_view(
    ...
    try:
        ...
        print(f"[TRACK-PRODUCT-VIEW] Error: {e}")
        await session.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/products/analytics/interaction")
async def track_product_interaction(
    data: dict,
    device: Device = Depends(get_current_device),
    session: AsyncSession = Depends(get_async_session)
):
    """Track product interactions (navigation, QR scans)"""
    try:
        ...
        print(f"[TRACK-PRODUCT-INTERACTION] Error: {e}")
        await session.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/devices/{device_id}/products/assign")
async def assign_products_to_device(
    device_id: str,
    data: dict,
    user: User = Depends(current_active_user),
    session: AsyncSession = Depends(get_async_session)
):
    """Assign products/categories/tags to a device"""
    try:
        # Clear existing assignments
        await session.execute(
            delete(ProductDeviceAssignment).where(ProductDeviceAssignment.device_id == device_id)
        )

        # Add new assignments
        for assignment in data.get('assignments', []):
            session.add(ProductDeviceAssignment(
                device_id=device_id,
                product_id=assignment.get('product_id'),
                category_id=assignment.get('category_id'),
                tag_id=assignment.get('tag_id'),
                display_order=assignment.get('display_order', 0)
            ))

        await session.commit()

        print(f"[ASSIGN-DEVICE-PRODUCTS] Assigned products to device {device_id}")
        return {"success": True}

    except Exception as e:
        print(f"[ASSIGN-DEVICE-PRODUCTS] Error: {e}")
        traceback.print_exc()
        await session.rollback()
        raise HTTPException(status_code=500, detail=str(e))


@app.post("/api/locations/{location_id}/products/assign")
async def assign_products_to_location(
    location_id: int,
    data: dict,
    user: User = Depends(current_active_user),
    session: AsyncSession = Depends(get_async_session)
):
    """Assign products/categories/tags to a location"""
    try:
        # Clear existing assignments
        await session.execute(
            delete(ProductLocationAssignment).where(ProductLocationAssignment.location_id == location_id)
        )

        # Add new assignments
        for assignment in data.get('assignments', []):
            session.add(ProductLocationAssignment(
                location_id=location_id,
                product_id=assignment.get('product_id'),
                category_id=assignment.get('category_id'),
                tag_id=assignment.get('tag_id'),
        ...
        print(f"[ASSIGN-LOCATION-PRODUCTS] Assigned products to location {location_id}")
        return {"success": True}
